...
### Verification Proof
- **Cloud Pub/Sub:** Messages verified in subscription `needflare-reports-sub` with attributes `category: water`, `sectorId: sector-alpha`.
- **Cloud Firestore:** Verified live documents in database `needflare-db` under collections `reports/test-001` and `tasks/task-genkit-*`.
- **Gemini 3.7 Flash:** Live multi-step rationale generated and returned with calculated disaster relief payload.
